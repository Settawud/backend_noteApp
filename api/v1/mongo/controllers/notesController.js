...
export const createNote = async (req, res) => {
  console.log("[CREATE NOTE] body:", req.body, "user:", req.user);
  const {
    title,
    content,
    tags = [],
  ...
  // ปลอดภัยกับหลายรูปแบบของ auth middleware
  const userId = req.user?.user?._id ?? req.user?._id ?? null;
  ...
  try {
    ...
    try {
      ...
      console.error("[CREATE NOTE] embedding error (ignored):", e?.message || e);
      embedding = null;
    }
    ...
    console.error("[CREATE NOTE] Error creating note:", err);
    return res.status(500).json({
      error: true,
      message: "Failed to create note",
      details: err.message,
